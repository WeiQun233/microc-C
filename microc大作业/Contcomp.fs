...
//执行局部优化的代码生成函数
//INCSP m : increase stack top by m
//RET m   :pop m and return to s[sp]
//Label lab 符号标签；伪指令。
...
let addLabel C : label * instr list =          (* 条件跳转到C *)
    match C with
    | Label lab :: _ -> (lab, C)
    | GOTO lab :: _  -> (lab, C)
    | _              -> let lab = newLabel() 
...
let makeJump C : instr * instr list =          (* 无条件跳转到C *)
    match C with
    | RET m              :: _ -> (RET m, C)
...
let rec deadcode C =    (* 去除C中的第一个Label及之前的所有的代码 *)
    match C with
    | []              -> []
    | Label lab :: _  -> C
    | _         :: C1 -> deadcode C1
...
let rec lookup env x =   (* env中查找名称为x的值 *)
    match env with 
