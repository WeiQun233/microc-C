...
and cExpr (e : expr) (varEnv : VarEnv) (funEnv : FunEnv) : instr list = 
    ...
    | PostInc acc    -> cAccess acc varEnv funEnv @ [DUP] @ [LDI] @ [SWAP] @ [DUP] @ [LDI] @ [CSTI 1] @ [ADD] @ [STI] @ [INCSP -1]
    | PostDec acc    -> cAccess acc varEnv funEnv @ [DUP] @ [LDI] @ [SWAP] @ [DUP] @ [LDI] @ [CSTI 1] @ [SUB] @ [STI] @ [INCSP -1]
    ...
    | AssignPrim(ope, acc, e) -> 
      ...
      cAccess acc varEnv funEnv @ [DUP] @ [LDI] @ cExpr e varEnv funEnv @ (getOpe ope) @ [STI]
    | Prim1(ope, e1) ->
    ...
    //   let labelse = newLabel()
    //   let labend = newLabel()
    //   cExpr e1 varEnv funEnv @ cExpr e2 varEnv funEnv @ [LT] @ [IFNZRO labelse]
    //   @ cExpr e1 varEnv funEnv @ [GOTO labend;Label labelse] @ cExpr e2 varEnv funEnv
    //   @ [Label labend]  
    // | Min(e1, e2)    ->
    //   let labelse = newLabel()
    //   let labend = newLabel()
    //   cExpr e1 varEnv funEnv @ cExpr e2 varEnv funEnv @ [LT] @ [IFNZRO labelse]
    //   @ cExpr e2 varEnv funEnv @ [GOTO labend;Label labelse] @ cExpr e1 varEnv funEnv
    //   @ [Label labend]
